...
//
// Akshay's and Wyland's Senior Project
// Implement Stochastic Gradient Descent 
// Create a combined metric of bandwidth use and fairness
// Remy uses loss = Ln(throughput) - importance_factor*ln(delay)
// predicted loss =  + [bias Ca Ss] * [1 wCa wSs]
// sum of squared residuals = (Perfect loss - Predicted loss)^2
// We need to take the derivative of (true - predicted)^2
// predicted = bias*1 + Ca*wCa + Ss*wSs
// gradient [bias#,wCa#,wSs#] = [bias ;Ca; Ss] ( [bias Ca Ss][1;Ca;Ss] -Perfect loss)
// we start with 1 and what do we extract, there are 4 flows. 
// Only 2 flows matter though, we don't care about ACK streams
...
//Akshay and Wyland's senior project
//loss = mean_throughput - importance_factor*std(mean_throughput)
double loss(std::vector<double> throughput, double importance_factor){
	double sum = std::accumulate(throughput.begin(),throughput.begin()+2,0.0);
	double mean = sum / (throughput.size() / 2);
	double sq_sum = std::inner_product(throughput.begin(),
			throughput.begin()+2,
			throughput.begin(),0.0);
	double stdev = std::sqrt(sq_sum /(throughput.size() / 2)- mean*mean); 
	//std::cout << "sum: " << sum << " mean: " << mean << std::endl;
	//std::cout << "sq_sum: " << sq_sum << " stdev: " << stdev << std::endl;
	//10 is so that we can use gradient descent
	double loss =5 -mean + (stdev * importance_factor);
	//std::cout << "loss: " << loss << std::endl;
	return loss;
}
//actual variables, weights, perfect loss
//WHEN I COME BACK:: I NEED TO FIX THIS;: y should not be 5, do not use
// prediciton and y, use perfect world, and current value.
std::vector<double> gradient(
		std::vector<double> X,
		std::vector<double> w){
	//double prediction = std::inner_product(X.begin(),X.end(),w.begin(),0.0);
	// we need to iterate through this so that I can see what the exact values are.
	double y = loss(X,1);
	double sum = 0;
	...
	//std::cout << "prediction: " << prediction << " sq_error: " << sq_error << std::endl;
	std::cout << "gradient: ";
	std::vector<double> grad = X;
	std::cout << "(prediction - 5)" << (sum - y) << std::endl;
	for(auto& element : grad){
		std::cout << element << " ";
		 element = -3;
	 }
	return grad;
...
double computeCost(
	...
	double y){
		...
		double prediction = std::inner_product(X.begin(),X.end(),w.begin(),0.0);
		double sq_error = std::pow(prediction - y,2);
	return sq_error;
}
// gradient [wCa#,wSs#] = [Ca; Ss] ( [Ca Ss][Ca;Ss] -Perfect loss)
...
main (int argc, char *argv[])
{
	...
	uint32_t pacingCaRatio = 120;
	uint32_t pacingSsRatio = 200;
	...
	cmd.AddValue("PacingCaRatio","Percent pacing rate increase for congestion avoidance conditions",pacingCaRatio);
	cmd.AddValue("PacingSsRatio","Percent pacing rate increase for slow start conditions",pacingSsRatio);
	...
	for(unsigned int i= 0 ; i < 10; i++){
		cmd.Parse (argc, argv);
		pacingCaRatio = w.at(0);
		pacingSsRatio = w.at(1);
		...
		Config::SetDefault("ns3::TcpSocketState::PacingCaRatio", UintegerValue(pacingCaRatio));
		Config::SetDefault("ns3::TcpSocketState::PacingSsRatio", UintegerValue(pacingSsRatio));
		...
		for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
		{
			...
			double throughput = i->second.rxBytes * 8.0 / simulationEndTime.GetSeconds () / 1000 / 1000;
			tp.push_back(throughput);
			//std::cout << "  Throughput: " << throughput << " Mbps\n";

		}
		std::cout << "throughput: ";
		for(uint16_t i = 0;i < tp.size();i++){
			std::cout << tp.at(i) << " ";
		}
		std::cout << std::endl;
		double y = loss(tp,1);
		std::cout << "pacingCaRatio: " << pacingCaRatio << " pacingSsRatio: " << pacingSsRatio << " loss: " << y  <<  std::endl;
		std::vector<double> X = {static_cast<double>(pacingCaRatio),static_cast<double>(pacingSsRatio)};
		std::vector<double> g = gradient(X,w);
		for(auto& i : g) std::cout << i << " ";
		std::cout << std::endl; 

		std::cout << "new weights: ";
		for(unsigned int i = 0; i < w.size();i++){
		       	w.at(i)-= std::round(learning_rate*g.at(i)); 
			std::cout << w.at(i)<< " ";
		}
		std::cout << std::endl;
		cwndStream.close ();
